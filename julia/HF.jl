...
function main()
    ...
    try
        with_logger(logger) do
            @info "Calculating $mol_path with $basis_set_name..."
            ...
            @debug "S overlap matrix:" S
            @debug "T kinetic energy matrix:" T
            ...
            eri_end = time()
            @debug "ERIs computed in $(@sprintf("%.3f", eri_end - eri_start)) seconds"

            # Core Hamiltonian
            H_core = T + V_nuc
            @debug "H_core matrix:" H_core
            ...
            for iteration = 1:max_iter
                ...
                if ΔE < ϵ_tol
                    ...
                    @debug "Time taken: $(@sprintf("%.3f", scf_end - scf_start)) seconds"
                    converged = true
                    break
                else
                    E_old = E_total
            ...
            @debug "Final orbital energies: $ϵ"
        end
    finally
        close(info_io)
        close(debug_io)
